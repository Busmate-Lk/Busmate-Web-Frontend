    ...
    <div className="w-1/5 bg-white p-6 overflow-y-auto">
      ...
      {assignmentError && (
        <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-xl">
          <div className="text-red-800 text-sm font-medium">⚠️ Assignment Error</div>
          <div className="text-red-600 text-sm mt-1">{assignmentError}</div>
        </div>
      )}
            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${selectedTrip && (!assignable || isAssigning) ? 'opacity-50 cursor-not-allowed' : ''}
                  ${selectedTrip && assignable && !isAssigning ? 'cursor-pointer hover:shadow-lg hover:border-blue-300 transform hover:scale-[1.02]' : 'cursor-default'}
                  ${assignable && selectedTrip && !isAssigning ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-white hover:bg-gray-50'}
                  shadow-sm
                `}
                onClick={() => {
                  if (selectedTrip && assignable && psp.id && !isAssigning) {
                    onAssignPsp(psp.id);
                  }
                }}
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  {selectedTrip && (
                    <div className={`text-xs px-3 py-1 rounded-full font-semibold flex items-center space-x-1 ${
                      isAssigning 
                      ...
                      {isAssigning ? (
                        ...
                        <span>{assignable ? '✓ Assignable' : '✗ Not Available'}</span>
                      )}
                    </div>
                  )}
                </div>
