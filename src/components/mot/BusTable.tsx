'use client';

import { Eye, Edit2, Trash2, ChevronUp, ChevronDown, Bus, User, Building2 } from 'lucide-react';
import type { BusResponse } from '@/lib/api-client/route-management';

interface BusTableProps {
  buses: BusResponse[];
  onView: (bus: BusResponse) => void;
  onEdit: (bus: BusResponse) => void;
  onDelete: (bus: BusResponse) => void;
  loading?: boolean;
  onSort?: (field: string, direction: 'asc' | 'desc') => void;
  sortBy?: string;
  sortDir?: 'asc' | 'desc';
}

export default function BusTable({ 
  buses, 
  onView, 
  onEdit, 
  onDelete, 
  loading = false,
  onSort,
  sortBy = 'ntc_registration_number',
  sortDir = 'asc'
}: BusTableProps) {
  const getStatusBadge = (status?: string) => {
    if (!status) return '';
    
    const baseClasses = 'px-2 py-1 rounded-full text-xs font-medium';
    switch (status.toLowerCase()) {
      case 'active':
        return `${baseClasses} bg-green-100 text-green-800`;
      case 'inactive':
        return `${baseClasses} bg-gray-100 text-gray-800`;
      case 'pending':
        return `${baseClasses} bg-yellow-100 text-yellow-800`;
      case 'cancelled':
        return `${baseClasses} bg-red-100 text-red-800`;
      default:
        return `${baseClasses} bg-gray-100 text-gray-800`;
    }
  };

  const handleSort = (field: string) => {
    if (onSort) {
      const newSortDir = sortBy === field && sortDir === 'asc' ? 'desc' : 'asc';
      onSort(field, newSortDir);
    }
  };

  const getSortIcon = (field: string) => {
    if (sortBy !== field) return null;
    return sortDir === 'asc' ? (
      <ChevronUp className="w-4 h-4" />
    ) : (
      <ChevronDown className="w-4 h-4" />
    );
  };

  const SortableHeader = ({ field, children }: { field: string; children: React.ReactNode }) => (
    <th 
      className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer hover:bg-gray-100 transition-colors"
      onClick={() => handleSort(field)}
    >
      <div className="flex items-center gap-1">
        {children}
        {getSortIcon(field)}
      </div>
    </th>
  );

  const formatDate = (dateString?: string) => {
    if (!dateString) return 'N/A';
    try {
      return new Date(dateString).toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'short',
        day: 'numeric',
      });
    } catch {
      return 'Invalid date';
    }
  };

  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900">Bus Fleet Directory</h3>
        <p className="text-sm text-gray-600 mt-1">
          {buses.length} buses found
        </p>
      </div>
      
      <div className="overflow-x-auto">
        <table className="w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <SortableHeader field="ntcRegistrationNumber">Bus Details</SortableHeader>
              <SortableHeader field="plateNumber">Registration</SortableHeader>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                Operator Info
              </th>
              <SortableHeader field="capacity">Capacity & Model</SortableHeader>
              <SortableHeader field="status">Status</SortableHeader>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                Actions
              </th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center">
                  <div className="flex items-center justify-center">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
                    <span className="ml-2 text-gray-500">Loading buses...</span>
                  </div>
                </td>
              </tr>
            ) : buses.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center">
                  <div className="text-gray-500">
                    <Bus className="w-16 h-16 mx-auto mb-4 text-gray-300" />
                    <div className="text-lg font-medium">No buses found</div>
                    <div className="text-sm">
                      Try adjusting your search criteria or add a new bus.
                    </div>
                  </div>
                </td>
              </tr>
            ) : (
              buses.map((bus) => (
                <tr key={bus.id} className="hover:bg-gray-50 transition-colors">
                  {/* Bus Details */}
                  <td className="px-6 py-4">
                    <div>
                      <div className="font-medium text-gray-900">
                        {bus.ntcRegistrationNumber || 'No NTC Registration'}
                      </div>
                      <div className="text-sm text-gray-500">
                        ID: {bus.id?.slice(-8) || 'N/A'}
                      </div>
                      <div className="text-sm text-gray-500">
                        Created: {formatDate(bus.createdAt)}
                      </div>
                    </div>
                  </td>

                  {/* Registration */}
                  <td className="px-6 py-4">
                    <div>
                      <div className="text-sm font-medium text-gray-900">
                        {bus.plateNumber || 'No Plate Number'}
                      </div>
                      <div className="text-sm text-gray-500">
                        Plate Number
                      </div>
                    </div>
                  </td>

                  {/* Operator Info */}
                  <td className="px-6 py-4">
                    <div className="space-y-1">
                      <div className="flex items-center text-sm text-gray-900">
                        <Building2 className="w-4 h-4 mr-2 text-gray-400" />
                        {bus.operatorName || 'Unknown Operator'}
                      </div>
                      <div className="text-sm text-gray-500">
                        ID: {bus.operatorId?.slice(-8) || 'N/A'}
                      </div>
                    </div>
                  </td>

                  {/* Capacity & Model */}
                  <td className="px-6 py-4">
                    <div>
                      <div className="flex items-center text-sm text-gray-900">
                        <User className="w-4 h-4 mr-1 text-gray-400" />
                        {bus.capacity || 0} seats
                      </div>
                      <div className="text-sm text-gray-500">
                        {bus.model || 'Model not specified'}
                      </div>
                    </div>
                  </td>

                  {/* Status */}
                  <td className="px-6 py-4">
                    <span className={getStatusBadge(bus.status)}>
                      {bus.status?.charAt(0).toUpperCase() + (bus.status?.slice(1) || '')}
                    </span>
                  </td>

                  {/* Actions */}
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => onView(bus)}
                        className="text-blue-600 hover:text-blue-900 p-1 rounded hover:bg-blue-50 transition-colors"
                        title="View Details"
                      >
                        <Eye className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => onEdit(bus)}
                        className="text-green-600 hover:text-green-900 p-1 rounded hover:bg-green-50 transition-colors"
                        title="Edit"
                      >
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => onDelete(bus)}
                        className="text-red-600 hover:text-red-900 p-1 rounded hover:bg-red-50 transition-colors"
                        title="Delete"
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                  </td>
                </tr>
              ))
            )}
          </tbody>
        </table>
      </div>
    </div>
  );
}
