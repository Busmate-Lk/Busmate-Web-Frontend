...
export default function TripsPage() {
    ...
    const loadFilterOptions = useCallback(async () => {
        try {
            const response: TripFilterOptionsResponse = await TripManagementService.getTripFilterOptions();
            ...
            setFilterOptions({
                ...
                buses: response.buses?.map(bus => ({
                    id: bus.id || '',
                    registrationNumber: bus.plateNumber || ''
                })) || [],
                passengerServicePermits: response.passengerServicePermits?.map(psp => ({
                    id: psp.id || '',
    ...
    const loadStatistics = useCallback(async () => {
        try {
            const response: TripStatisticsResponse = await TripManagementService.getTripStatistics();
    ...
    }, [statusFilter, routeFilter, operatorFilter, scheduleFilter, busFilter, pspFilter, fromDate, toDate, hasPsp, hasBus, hasDriver, hasConductor]);
    ...
    }, [statusFilter, routeFilter, operatorFilter, scheduleFilter, busFilter, pspFilter, fromDate, toDate, hasPsp, hasBus, hasDriver, hasConductor, updateQueryParams]);
    ...
    const handleGenerateTrips = () => {
        router.push('/mot/trips/generate');
    };
    ...
    const handleCancelConfirm = async () => {
        ...
        try {
            ...
            await TripManagementService.cancelTrip(tripToCancel.id || '', cancelReason || 'Cancelled by user');
            loadTrips(); // Reload data
            loadStatistics(); // Update stats
        ...
        >
            <div className="space-y-6">
                ...
                <TripActionButtons
                    ...
                    onBulkAssignPsp={selectedTrips.length > 0 ? handleBulkAssignPsp : undefined}
                    onBulkStart={selectedTrips.length > 0 ? handleBulkStart : undefined}
                    onBulkComplete={selectedTrips.length > 0 ? handleBulkComplete : undefined}
                ...
                {showDeleteModal && tripToDelete && (
                    <DeleteConfirmationModal
                        ...
                        title="Delete Trip"
                        itemName={`${tripToDelete.routeName} - ${formatDate(tripToDelete.tripDate)}`}
