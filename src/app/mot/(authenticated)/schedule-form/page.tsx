...
export default function EnhancedScheduleForm() {
  ...
  const [scheduleType, setScheduleType] = useState<'forward' | 'backward'>('forward');
  ...
  useEffect(() => {
    if (isEditMode && scheduleId) {
      const existingSchedule = dummySchedules.find(schedule => schedule.id === scheduleId);
      
      if (existingSchedule) {
        setRouteName(String(existingSchedule.routeName));
        setRouteCategory(existingSchedule.routeGroupName.toLowerCase().includes('highway') ? 'expressway' : 'normal');
        setScheduleType(String(existingSchedule.scheduleType) as 'forward' | 'backward');
        ...
        if (existingSchedule.scheduleStops.length > 0) {
          ...
          const lastStop = existingSchedule.scheduleStops[existingSchedule.scheduleStops.length - 1];
        ...
        const formattedStops: StopSchedule[] = existingSchedule.scheduleStops.map(stop => ({
        ...
        setValidFrom(existingSchedule.effectiveStartDate.toISOString().split('T')[0]);
        setValidUntil(existingSchedule.effectiveEndDate.toISOString().split('T')[0]);
  ...
  const handleSave = () => {
    ...
    if (!routeNumber || !routeName || !routeCategory || !startPoint || !endPoint || !startTime || !endTime || !validFrom || !validUntil) {
    ...
    const incompleteStops = stopSchedules.filter(stop => 
      !stop.stopId || !stop.arrivalTime || !stop.departureTime
    );
    
    if (incompleteStops.length > 0) {
      alert("Please complete all stop details (stop selection, arrival time, and departure time).");
      return;
    }
    ...
    const schedule = {
      ...
      name: `${routeName} - ${scheduleType === 'forward' ? startPoint + ' to ' + endPoint : endPoint + ' to ' + startPoint}`,
      routeId: routeNumber,
      routeName,
      routeGroupId: 'RG001', // This would be selected or generated
      routeGroupName: routeCategory === 'expressway' ? 'Highway Routes' : 'Standard Routes',
      scheduleType,
